...
function validateLoginForm() {
    ...
    let error_login_user = document.getElementById('login-error-user');
    let error_login_password = document.getElementById('login-error-password');

    if (input_user == '') {
        error_login_user.innerHTML = 'Username required';
        setTimeout(() => {
            error_login_user.innerHTML = "";
        }, 3000);
        return false;
    }
    if (input_password == '') {
        error_login_password.innerHTML = "Password required";
        setTimeout(() => {
